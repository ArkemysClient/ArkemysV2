...
public class GuiAccountSelector extends GuiScreen {
	...
	public void initGui() {
		...
		buttonList.add(reloadskins = new GuiButton(0, 2, 2, 120, 20, "Recharger skins"));
		buttonList.add(new GuiButton(1, this.width / 2 + 4 + 40, this.height - 52, 120, 20, "Ajouter"));
		buttonList.add(login = new GuiButton(2, this.width / 2 - 154 - 10, this.height - 52, 120, 20, "Connexion"));
		buttonList.add(edit = new GuiButton(3, this.width / 2 - 40, this.height - 52, 80, 20, "Modifier"));
		buttonList.add(loginoffline = new GuiButton(4, this.width / 2 - 154 - 10, this.height - 28, 110, 20, "Connexion" + " " + "(Crack)"));
		buttonList.add(new GuiButton(5, this.width / 2 + 4 + 50, this.height - 28, 110, 20, "Retour"));
		buttonList.add(delete = new GuiButton(6, this.width / 2 - 50, this.height - 28, 100, 20, "Supprimer"));
		search = new SuggestorTextField(7, this.fontRendererObj, this.width / 2 - 80, 14, 160, 16, "Chercher");
	    updateButtons();
	...
	public void drawScreen(int mx, int my, float delta) {
		...
		drawCenteredString(fontRendererObj, "Choisir un compte", this.width / 2, 4, -1);
		if (error != null) {
		...
		if (accountsgui.selected() >= 0) {
			...
			if (acc.online()) drawString(fontRendererObj, "Premium", width - 8 - 61, height / 2 - 64 - 13, 6618980);
			else drawString(fontRendererObj, "Crack", width - 8 - 61, height / 2 - 64 - 13, 16737380);
			drawString(fontRendererObj, "Fois utilisé", width - 8 - 61, height / 2 - 64 - 15 + 12, -1);
			drawString(fontRendererObj, String.valueOf(acc.uses()), width - 8 - 61, height / 2 - 64 - 15 + 21, -1);
			if (acc.uses() > 0) {
				drawString(fontRendererObj, "Dernière utilisation", width - 8 - 61, height / 2 - 64 - 15 + 30, -1);
	 ...
	 */
	private void add() {
		mc.displayGuiScreen(new AbstractAccountGui(this, "Ajouter", acc -> {
	...
	public class AccountList extends GuiListExtended {
		...
		public void edit() {
			...
			mc.displayGuiScreen(new AbstractAccountGui(GuiAccountSelector.this, "Modifier", acc -> {
		...
		public void delete() {
			...
			}, "Supprimer un compte", "Supprimer " + acc.alias() + " ?", 0));
		}
		...
		public ResourceLocation model(boolean forceReload) {
			...
			if (modelTexture == null) {
				...
				try {
					...
					Client.warn("Unable to bake skin model: " + account.alias(), t);
		...
		public ResourceLocation face(boolean forceReload) {
			...
			if (faceTexture == null) {
				...
				try {
					...
					Client.warn("Unable to bake skin face: " + account.alias(), t);
