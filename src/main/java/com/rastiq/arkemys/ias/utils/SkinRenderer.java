			...
			try {
				...
				if (uuid == null) {
					try (InputStreamReader isr = new InputStreamReader(new URL("https://api.mojang.com/users/profiles/minecraft/" + name).openStream(), StandardCharsets.UTF_8)) {
						uuid = UUIDTypeAdapter.fromString(Client.GSON.fromJson(isr, JsonObject.class).get("id").getAsString());
					}
				}
				String base64Data;
				try (InputStreamReader isr = new InputStreamReader(new URL("https://sessionserver.mojang.com/session/minecraft/profile/" + UUIDTypeAdapter.fromUUID(uuid)).openStream(), StandardCharsets.UTF_8)) {
					base64Data = Client.GSON.fromJson(isr, JsonObject.class).getAsJsonArray("properties").get(0).getAsJsonObject().get("value").getAsString();
				}
				JsonObject jo = Client.GSON.fromJson(new String(Base64.getDecoder().decode(base64Data), StandardCharsets.UTF_8), JsonObject.class).getAsJsonObject("textures").getAsJsonObject("SKIN");
				...
				try {
					...
					th.addSuppressed(t);
					Client.warn("Unable to load skin: " + name, th);
					return;
				}
			}
			try {
				...
				Client.warn("Unable to make model of skin: " + name, t);
			}
		}
		if (!faceF.exists()) {
			...
			try {
				...
				Client.warn("Unable to make face from skin model: " + name, t);
			}
		}
	}

	public static class SkinsLoadingScreen extends GuiScreen {
		private static final ResourceLocation PROGRESS = new ResourceLocation("arkemys/nametags/icon.png");
