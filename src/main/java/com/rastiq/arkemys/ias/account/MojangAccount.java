...
import com.rastiq.arkemys.ias.utils.Request;
...
public class MojangAccount implements Account {
	...
	public void login(Minecraft mc, Consumer<Throwable> handler) {
		new Thread(() -> {
			...
			mc.addScheduledTask(() -> {
				((SessionAccessor) Minecraft.getMinecraft()).setSession(new Session(username, UUIDTypeAdapter.fromUUID(uuid), accessToken, "mojang"));
				handler.accept(null);
			});
